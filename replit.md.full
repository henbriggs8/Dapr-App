...
- April 25, 2026. Merged `/services` into `/booking`. The polished accordion tier picker (Essential Wash / Interior Detail / Refresh Detail with icons + Included/Best for/Good to know expanded sections + disclaimer) now lives directly in `client/src/pages/booking-screen.tsx` as the first step, followed by the existing time picker (Now/Schedule modes) and `BookingDialog` modal flow. Selecting a tier highlights the row (purple ring/icon swap) and smooth-scrolls to the When section. Deleted `client/src/pages/services-page.tsx`. `/services` route now `<Redirect to="/booking" />` in `App.tsx`. Updated `home-desktop.tsx` (`goServices` → `/booking`) and `corporate-desktop.tsx` Services nav link → `/booking`. The `/services` route is reserved for a future marketing page.
- April 25, 2026. Address autocomplete on the desktop home hero "Vehicle location" input. Uses Photon (komoot.io, OSM-backed, no API key) with 250ms debounce, ≥3 char trigger, suggestion dropdown styled to match the dark glass UI, full keyboard nav (↑/↓/Enter/Esc), outside-click close. State + effect implemented inline in `client/src/pages/home-desktop.tsx`.
