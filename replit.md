...
- **April 25, 2026** — Premium WebGL fluid background on the desktop hero (`client/src/pages/home-desktop.tsx`). New `client/src/components/fluid-hero-background.tsx` renders an OGL-powered fragment shader (domain-warped fbm simplex noise) using the brand palette (#050506 black, #8c52ff purple, #11204a deep blue, #3a86ff cool blue, #22e1ff aqua, #e8eef5 silver-white) with slow ambient flow, gloss highlights, vignette, and grain. Replaced the static purple radial blob in the hero with the shader + a top-to-bottom black gradient overlay so headline contrast stays AAA and the section blends into the next one. Honors `prefers-reduced-motion` (single static frame), pauses on tab-hidden + when scrolled off-screen via IntersectionObserver, caps DPR at 1.5, and falls back to a CSS radial-gradient if WebGL fails. Added `ogl` (~8KB gzipped) to dependencies.
